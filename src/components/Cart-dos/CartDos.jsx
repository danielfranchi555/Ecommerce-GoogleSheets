...
import {Stack,Drawer, DrawerBody, DrawerCloseButton, DrawerContent, DrawerHeader, DrawerOverlay, useDisclosure, Button, Text, TableContainer, Table, Thead, Tr, Th, Tbody, Td, Image, Link, HStack,} from '@chakra-ui/react'
...
export const CartDos = () => {
  ...
  return (
    <Stack onClick={onOpen} align='flex-end'  className='fixed-bottom mb-1 container-lg'>
                   <HStack>
  ...
  <Table variant='simple'>
    ...
    <Tbody>
      {cart.map((item)=>(
        ...
        <Td><Image boxSize='50px' src={item.image}/> </Td>
        <Td>{item.title}</Td>
        <Td>{item.quantity}</Td>
        <Td> ${subtTotal(parseInt(item.price), item.quantity)} </Td>
          ...
          <Stack  direction='row'  justifyContent='flex-end' >
            <Stack  direction='row' >
            <Text   px={1} fontSize={20}>Precio Total : $ <span style={{fontWeight:'700'}}> {Total()} </span>  </Text>
                <Button variant='outline' colorScheme='red' mr={3} onClick={onClose}>
              Cancel
            </Button>
            <Button as={Link} href={`https://wa.me/543512002606?text=${encodeURIComponent(PagoTerminado)}`} colorScheme='blue'>Terminar Compra</Button>
            </Stack>
